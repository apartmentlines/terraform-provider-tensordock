...
Manages a TensorDock virtual machine instance using the documented public instance creation and instance management endpoints.
...
- `location_id` (String) TensorDock location UUID for location-based deployment.
...
- `gpu_type` (String) TensorDock GPU model `v0Name`.
- `gpu_count` (Number) Number of GPUs of `gpu_type`.
- `ssh_public_key` (String) SSH public key injected during instance creation.
...
- `port_forwards` (List of Objects) Port forwards returned by TensorDock.
